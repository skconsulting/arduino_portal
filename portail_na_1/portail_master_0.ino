...
int opendelay = 1000; // 1 second afte command to open
int closedelay = 0; // 0 second after command to close
double maxVCC = 1000.0; // overdrive Voltage mv
...
void loop() {
  ...
  POC.update();
  curPOC = millis();
  if (curPOC - startPOC > rotStop * 1000)
  {
    startPOC = millis();
    if (!POC.read()) {
