...
char pass[] = "tagada1956";        // your network password
int status = WL_IDLE_STATUS;     // the Wifi radio's status
int role = 0;
String Pgarage = "Wcd5KPhDxCObUdw6";
...
void setup()
{
  ...
  Serial.begin(9600);
  Serial.println("Start rx433_wifi");
...
void loop()
{
  ...
  httpRequest(Pgarage, role);
  delay(delai);
  httpRequest(Pportail, role);
  delay(delai);

  httpRequest(Ptemp, role + 0.5);
  delay(delai);
